...
public class BudgetsController : ControllerBase
{
    ...
    {
        var userId = GetCurrentUserId();
        var status = await _budgetService.GetBudgetStatusAsync(year, month, userId);
        return Ok(status);
    }

    [HttpPost]
    public async Task<ActionResult<BudgetResponseDto>> PostBudget(CreateBudgetDto dto)
    {
        var userId = GetCurrentUserId();
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> PutBudget(Guid id, UpdateBudgetDto dto)
    {
        var userId = GetCurrentUserId();
        var success = await _budgetService.UpdateBudgetAsync(id, dto, userId);
